...
export class SingularValueDecomposition {
    ...
    public constructor(Arg : Matrix) {
        ...
        this.s = (s => { let a=[]; while(s-->0) a.push(0); return a; })(Math.min(this.m + 1, this.n));
        this.U = <any> (function(dims) { let allocate = function(dims) { if(dims.length==0) { return 0; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([this.m, nu]);
        ...
        let e : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.n);
        let work : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.m);
