...
export class EigenvalueDecomposition {
    ...
    /*private*/ cdivr : number;

    /*private*/ cdivi : number;

    /*private*/ cdiv(xr : number, xi : number, yr : number, yi : number) {
        let r : number;
        let d : number;
    ...
    public constructor(Arg : Matrix) {
        ...
        this.V = <any> (function(dims) { let allocate = function(dims) { if(dims.length==0) { return 0; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([this.n, this.n]);
        this.d = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.n);
        this.e = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.n);
        ...
        if(this.issymmetric) {
            ...
            this.H = <any> (function(dims) { let allocate = function(dims) { if(dims.length==0) { return 0; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([this.n, this.n]);
            this.ort = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.n);
            for(let j : number = 0; j < this.n; j++) {
                for(let i : number = 0; i < this.n; i++) {
                    this.H[i][j] = A[i][j];
                };
