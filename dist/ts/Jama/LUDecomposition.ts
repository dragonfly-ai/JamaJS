...
export class LUDecomposition {
    ...
    public constructor(A : Matrix) {
        ...
        this.piv = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.m);
        for(let i : number = 0; i < this.m; i++) {
            this.piv[i] = i;
        };
        this.pivsign = 1;
        let LUrowi : number[];
        let LUcolj : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.m);
    ...
    public getPivot() : number[] {
        let p : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.m);
        for(let i : number = 0; i < this.m; i++) {
            p[i] = this.piv[i];
        };
        return p;
    ...
    public getDoublePivot() : number[] {
        let vals : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.m);
        for(let i : number = 0; i < this.m; i++) {
