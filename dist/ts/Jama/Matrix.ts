...
export class Matrix {
    ...
    public constructor(A? : any, m? : any, n? : any) {
        if(((A != null && A instanceof <any>Array && (A.length==0 || A[0] == null ||A[0] instanceof Array)) || A === null) && ((typeof m === 'number') || m === null) && ((typeof n === 'number') || n === null)) {
            ...
            (() => {
                this.m = m;
                this.n = n;
                this.A = <any> (function(dims) { let allocate = function(dims) { if(dims.length==0) { return 0; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([m, n]);
            ...
            (() => {
                ...
                this.A = <any> (function(dims) { let allocate = function(dims) { if(dims.length==0) { return 0; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([m, this.n]);
                for(let i : number = 0; i < m; i++) {
    ...
    public getArrayCopy() : number[][] {
        let C : number[][] = <any> (function(dims) { let allocate = function(dims) { if(dims.length==0) { return 0; } else { let array = []; for(let i = 0; i < dims[0]; i++) { array.push(allocate(dims.slice(1))); } return array; }}; return allocate(dims);})([this.m, this.n]);
        for(let i : number = 0; i < this.m; i++) {
            for(let j : number = 0; j < this.n; j++) {
                C[i][j] = this.A[i][j];
            };
    ...
    public getColumnPackedCopy() : number[] {
        let vals : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.m * this.n);
        for(let i : number = 0; i < this.m; i++) {
            for(let j : number = 0; j < this.n; j++) {
    ...
    public times$Jama_Matrix(B : Matrix) : Matrix {
        ...
        let C : number[][] = X.getArray();
        let Bcolj : number[] = (s => { let a=[]; while(s-->0) a.push(0); return a; })(this.n);
     ...
     * @private
     */
    /*private*/ checkMatrixDimensions(B : Matrix) {
